...
const PharmacyInventory = () => {
    ...
    const handleDeleteItem = async (item) => {
        if (!window.confirm(`Are you sure you want to delete ${item.drug?.name} from inventory?`)) {
            return;
        }
    ...
    const getStockStatus = (item) => {
        ...
        if (isExpiringSoon(item)) {
            return { label: 'Expiring Soon', color: 'yellow', icon: AlertTriangle };
        }
        if (isLowStock(item)) {
    ...
    const filteredInventory = inventory.filter(item => {
        ...
        if (stockFilter === 'low_stock') {
            matchesStock = isLowStock(item);
        } else if (stockFilter === 'expiring') {
            matchesStock = isExpiringSoon(item);
        } else if (stockFilter === 'expired') {
            matchesStock = isExpired(item);
        }
    ...
    const getStats = () => {
        ...
        const lowStockItems = inventory.filter(item => isLowStock(item)).length;
        const expiringItems = inventory.filter(item => isExpiringSoon(item)).length;
        const expiredItems = inventory.filter(item => isExpired(item)).length;
                            ...
                            <form onSubmit={handleCreateItem} className="space-y-4">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="md:col-span-2">
                                        ...
                                        >
                                            <option value="">
                                                {loadingDrugs ? 'Loading drugs...' : 'Select Drug'}
                                            </option>
                                            {drugs.map((drug) => (
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            value={createFormData.quantity_available}
                                            onChange={handleCreateInputChange}
                                            min="0"
                                    ...
                                    <div>
                                        ...
                                        <input
                                            type="number"
                                            name="price_per_unit"
                                            value={createFormData.price_per_unit}
                                            onChange={handleCreateInputChange}
                                            min="0"
                                            step="0.01"
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            value={createFormData.low_stock_threshold}
                                            onChange={handleCreateInputChange}
                                            min="0"
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="bg-gray-50 p-4 rounded-lg space-y-3">
                                        <div className="flex justify-between">
                                            ...
                                            <span className="text-sm text-gray-900">{selectedItem.quantity_available} units</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span className="text-sm font-medium text-gray-600">Price per Unit:</span>
                                            <span className="text-sm text-gray-900">{formatCurrency(selectedItem.price_per_unit)}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span className="text-sm font-medium text-gray-600">Total Value:</span>
                                            <span className="text-sm font-bold text-gray-900">
                                                {formatCurrency(selectedItem.quantity_available * parseFloat(selectedItem.price_per_unit))}
                                            </span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span className="text-sm font-medium text-gray-600">Low Stock Threshold:</span>
                                            <span className="text-sm text-gray-900">{selectedItem.low_stock_threshold}</span>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div className="bg-gray-50 p-4 rounded-lg space-y-3">
                                        <div className="flex justify-between">
                                            ...
                                            <span className="text-sm text-gray-900">
                                                {drugCategories.find(c => c.value === selectedItem.drug?.category)?.label || selectedItem.drug?.category}
                                            </span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span className="text-sm font-medium text-gray-600">Dosage Form:</span>
                                            <span className="text-sm text-gray-900">{selectedItem.drug?.dosage_form}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span className="text-sm font-medium text-gray-600">Manufacturer:</span>
                                            <span className="text-sm text-gray-900">{selectedItem.drug?.manufacturer}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span className="text-sm font-medium text-gray-600">Prescription Required:</span>
                                            <span className="text-sm text-gray-900">
                                                {selectedItem.drug?.requires_prescription ? 'Yes' : 'No'}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        {isExpired(selectedItem) && (
                                            <div className="bg-red-50 p-3 rounded-lg border border-red-200">
                                                <div className="flex items-center">
                                                    ...
                                                    <span className="text-sm font-medium text-red-800">Product Expired</span>
                                                </div>
                                        ...
                                        {!isLowStock(selectedItem) && !isExpiringSoon(selectedItem) && !isExpired(selectedItem) && (
                                            <div className="bg-green-50 p-3 rounded-lg border border-green-200">
                                                <div className="flex items-center">
                                                    ...
                                                    <span className="text-sm font-medium text-green-800">Stock OK</span>
                                                </div>
